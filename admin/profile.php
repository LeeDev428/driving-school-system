...
$user_data = [];
$sql = "SELECT full_name, email, contact_number, profile_image FROM users WHERE id = ?";
            ...
            if (empty($full_name) || empty($email)) {
                ...
                if ($stmt = mysqli_prepare($conn, $check_sql)) {
                    ...
                    if (mysqli_stmt_num_rows($stmt) > 0) {
                        ...
                        // Update profile
                        $update_sql = "UPDATE users SET full_name = ?, email = ?, contact_number = ? WHERE id = ?";
                        if ($update_stmt = mysqli_prepare($conn, $update_sql)) {
                            mysqli_stmt_bind_param($update_stmt, "sssi", $full_name, $email, $contact_number, $user_id);
